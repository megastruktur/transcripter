...
- Pre-flight (`cmd_pre_flight`) открывает и пробует каждый выбранный
  ...
  явно выключен.
- Миксер (`spawn_mixer`) пишет **сырой interleaved i16 PCM** в `<id>.pcm`
  на диск (RAM плоская, запись переживает часы работы). Канальность
  следует источникам: dual-tap → **стерео** (mic→L, system→R,
  интерливинг без суммирования — источники не смешиваются, сервер
  разведёт их обратно); только микрофон → моно, как раньше.
  Layout фиксируется на старте: деградация system-тапа не меняет
  канальность файла (R становится цифровой тишиной), фрейм-каунт не
  съезжает.
...
```
audio.flac                     ← канонический вход (mono; для записей
                                  dual-tap клиента — stereo: mic→L, system→R)
meta/
  channels/                    ← stereo только (chunk-стадия): split_channels
    mic.flac, system.flac      ← per-source mono (удаляются после merge;
                                  пере-выводимы из audio.flac, сплит идемпотентен)
  chunks/                      ← chunk-стадия (chunk.enabled=true)
    chunks.json                ← манифест: граница crash/resume (mono)
    mic/chunks.json            ← per-channel манифесты (stereo): ОДИН план
    system/chunks.json           нарезки на оба канала — чанк N mic и system
  ...
  diarization.json             ← сегменты спикеров (diarize); stereo: спикеры
                                  имена́ "mic:spk_N"/"system:spk_N" (глобальный
                                  namespace — каналы не пересекаются)
  diarized-transcript.md       ← реплики по спикерам (merge_speakers);
                                  stereo: атрибуция channel-first — слово mic
                                  не может получить спикера system
...
**Stereo (dual-tap) путь**: записи клиента с system-audio приходят как
стерео (mic→L, system→R, см. client recording.rs `mix_samples`).
`split_channels` (ffmpeg channelsplit, один проход, оба выхода) кладёт
per-source mono в `meta/channels/`; нарезка идёт per-channel в
`chunks/{mic,system}/` с **одним планом** на оба канала — чанк N обоих
каналов покрывает один и тот же интервал. Наличие `meta/channels/mic.flac`
— маркер stereo для всех нижестоящих стадий (`channel_names`). Моно-записи
не меняют ни одного пути.
...
- Stereo (dual-tap): per-channel chunked-прогон (mic, затем system —
  последовательно, тот же CPU voice-stack), выход — **единый
  хронологический** `segments.json`; каждое слово/сегмент несёт
  `"channel": "mic"|"system"` (legacy-моно артефакты — без поля).
...
1. Каждое **слово** получает спикера диаризационного сегмента с
   **максимальным перекрытием** по времени. Если перекрытия нет вообще —
   ...
   Stereo: атрибуция **channel-first** — слово канала mic ищет спикера
   только среди `mic:*`-сегментов (system — среди `system:*`), так что
   слово mic никогда не получит спикера system, даже если тот
   перекрывает его сильнее. Нет префиксных сегментов (legacy-моно
   диаризация) → фолбэк на все сегменты: канал не осиротит слово.
