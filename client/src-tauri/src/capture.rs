...
//
// Desktop (linux / macOS / windows) uses cpal + OS-native host APIs:
//   linux  → ALSA (cpal-alsa)
//   macOS  → CoreAudio (cpal-coreaudio) — system loopback via
//            capture_macos::create_loopback (AVAudioEngine tap)
//   windows→ WASAPI loopback (capture_windows)
//
// Android deliberately has NO native capture path: the android tauri shell
// uses the system WebView (AndroidX WebKit, see gen/android/.../MainActivity.kt)
// and recording goes through `navigator.mediaDevices.getUserMedia({ audio: true })`
// in the SvelteKit front-end. See client/ANDROID_POC.md for the getUserMedia
// evaluation and the permission-request plumbing that has to be added to
// TauriActivity.onPermissionRequest for it to actually work.
//
// This keeps the desktop Rust crate compilable for the android target
// (`pnpm tauri android build --debug`) without dragging cpal's `-laaudio` link
// line into the NDK toolchain. The two #[cfg(...)] blocks below guard
// every cpal-backed function; the trailing `rms` helper is cross-platform and
// stays for the front-end to compute levels from samples that already came
// off getUserMedia (when we wire that up).
...
// Android stubs. The recording path runs in the WebView via getUserMedia;
// these stubs exist only so that recording.rs / permissions.rs / lib.rs
// compile unchanged for the android target. Recording start paths that try to
// open a native stream return an error (the UI never reaches them on android).
//
// IMPORTANT: this stub intentionally does NOT expose any cpal type — cpal is
// not in the android [dependencies] block of Cargo.toml and the linker would
// reject the build. The fields here mirror the desktop shape (channels /
// sample_rate / buffer_size as plain scalars) so recording.rs's
// `stream.config.sample_rate()` / `.channels()` access patterns type-check.
// At runtime on android those code paths are unreachable because
// open_mic_stream / open_system_stream always return Err.
#[cfg(target_os = "android")]
pub mod android_stub {
    use serde::Serialize;
    ...
    impl CapturedStream {
        pub fn drain_into(&self, _out: &mut Vec<f32>, _limit: usize) -> usize {
            0
        }
        pub fn dropped(&self) -> u64 {
            0
    ...
    pub fn list_devices() -> Result<AudioDevices, String> {
        Ok(AudioDevices {
            microphones: Vec::new(),
            system_outputs: Vec::new(),
            default_microphone: None,
            default_system_output: None,
        })
    }

    pub fn open_mic_stream(_id: Option<&str>) -> Result<CapturedStream, String> {
        Err("native mic capture is disabled on Android; use the WebView getUserMedia path".into())
    }

    pub fn open_system_stream(_id: Option<&str>) -> Result<CapturedStream, String> {
        Err("native system-audio capture is unsupported on Android".into())
    }
}
...
/// Cross-platform RMS — used by the front-end and any future level metering.
