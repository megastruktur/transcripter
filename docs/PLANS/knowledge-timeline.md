...
- Recap-ретрив по семантическому индексу: вместо/вместе с digest
  ...
  СТАТУС (2026-08-30, коммит 5821b98): РЕАЛИЗОВАНО. build_recap
  теперь дописывает к digest-note блок «Related earlier discussion»:
  KNN по индексу тега, query = первое окно транскрипта, текущая
  запись исключена (порядок стадий summarize→enrich — на regenerate
  свои сегменты уже в индексе дословно). Ручки recap_k=6 /
  recap_budget_chars=1600; over-fetch фиксированный 200 (k*4
  голодал до нуля на daily blob). Live: pathfinder → 6 prior hits,
  daily blob → digest-only (0 пост-self хитов, корректно).
- Тематическая сегментация (главы длинных сессий) — идея №3 из
  BERT-таблицы. СТАТУС (2026-08-30): ОТЛОЖЕНО — потребитель отсутствует:
  Events-таб уже даёт осмысленные (LLM) главы с кликом и seek;
  change-point дублировал бы представление без адресуемости.
- Кросс-энкодер bge-reranker для серой зоны дедупа — идея №4.
  СТАТУС (2026-08-30): ОТЛОЖЕНО — серая зона уже трёхслойная
  (cosine-зоны → LLM Y/N → merge-on-error), user_corrected-узлы
  неуязвимы (фаза 4); mismerge-жалоб нет, второй ML-контейнер
  не окупается.
...
Sanity-вердикты 2026-08-30 по нереализованным идеям (подробное
обоснование — в статусах «Ближайших хвостов» выше):

| # | Идея | Модель | Вердикт 2026-08-30 |
|---|------|--------|-----------|
| 1 | Дедуп сущностей: эмбеддинг-префильтр | bge-m3 ONNX int8 | ✅ сделано (фаза 2.5) |
| 2 | Семантический поиск по сессиям неймспейса | та же | ✅ сделано (3.5 + 3.75 global) |
| 3 | Тематическая сегментация (главы) | sentence embeddings + change-point | ⛔ отложено — Events-таб уже даёт LLM-главы с seek |
| 4 | Кросс-энкодер для серой зоны дедупа | bge-reranker-v2-m3 | ⛔ отложено — зона уже 3-слойная, mismerge-жалоб нет |
| 5 | Автотегирование типа записи | ruBERT/XML-R | ⛔ не надо — тип кликается до записи, классификатор ошибается где юзер нет |
| 6 | Action items из сырых сегментов | fine-tuned token cls | ⛔ не надо — уже в enrich-онтологии (тип action_item) |
| 7 | Спикер-рекластеризация | sentence embeddings | ⛔ не надо — pyannote в стеке, запроса нет |
