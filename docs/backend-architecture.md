...
- **transcribe** — без ретраев (`maximum_attempts=1`): повтор = повторный
  ...
  Temporal согласованы и масштабируются от длительности аудио:
  `timeout_for`/`budget_transcribe` = 300 с + 40 с/мин аудио, при неизвестной
  длительности — из расчёта 2.5 ч максимума. Возобновление — только вручную
  из UI (regenerate).
- **diarize** — best-effort: retry ×4 c интервалом 30–60 с (поглощает
  холодный старт внешнего сервиса); бюджет 300 с + 40 с/мин; при исчерпании
  ретраев запись остаётся годной — transcript-only, стадия помечается
  `failed` + `last_error` для UI.
- **transcribe/diarize** шлют Temporal-heartbeat каждые 60 с
  (`heartbeat_timeout` 120 с) и исполняются в `asyncio.to_thread` — долгий
  блокирующий HTTP больше не замораживает event loop воркера.
