...
_DIGEST_PROMPT_HEADER = (
    ...
    "2. Returning names: people, places and items that appear across "
    ...
    "{language_directive}"
    "{context_section}"
    "Do not use timestamps or timecodes — no [hh:mm:ss] markers anywhere "
    "in the digest; the ts values below are for ordering only.\n"
    "Markdown only, no frontmatter — the host prepends its own.\n\n"
    "---\n\n"
    "Tag: {tag}\n"
    "Sessions: {sessions} (count: {count})\n"
    "Recurring entries (name — kind — sessions seen — dossier):\n{entities}\n\n"
